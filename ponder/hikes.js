...
function search(){
    ...
    let filterHikes = hikes.filter(function(hike){
        return (
            hike.name.toLowerCase().includes(hikeQuery.toLowerCase()) || hike.description.toLocaleLowerCase().includes(hikeQuery.toLowerCase()) || hike.tags.find(tag => tag.toLowerCase().includes(hikeQuery.toLocaleLowerCase()))
        );
    })

    console.log(filterHikes);

    let sortedHike = filterHikes.sort(compareHikes)
    ...
    sortedHike.forEach(function(hike){
        renderHike(hike);
    })
}
